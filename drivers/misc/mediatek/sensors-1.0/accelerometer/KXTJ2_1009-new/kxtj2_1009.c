/* KXTJ2_100922E motion sensor driver
 *
...
#define COMPATIABLE_NAME "mediatek,kxtj2_1009e_new"
...
static int KXTJ2_1009_i2c_write_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
...
static int KXTJ2_1009_ReadData(struct i2c_client *client, s16 data[KXTJ2_1009_AXES_NUM])
{
	...
	u8 addr = KXTJ2_1009_REG_DATAXLOW;
	...
	if (NULL == client) {
		err = -EINVAL;
	} else if (KXTJ2_1009_i2c_read_block(client, addr, buf, 0x06)) {
		GSE_ERR("error: %d\n", err);
	} else {
		data[KXTJ2_1009_AXIS_X] = (s16) buf[KXTJ2_1009_AXIS_X * 2] >> 4 | buf[KXTJ2_1009_AXIS_X * 2 + 1] << 4;
		data[KXTJ2_1009_AXIS_Y] = (s16) buf[KXTJ2_1009_AXIS_Y * 2] >> 4 | buf[KXTJ2_1009_AXIS_Y * 2 + 1] << 4;
		data[KXTJ2_1009_AXIS_Z] = (s16) buf[KXTJ2_1009_AXIS_Z * 2] >> 4 | buf[KXTJ2_1009_AXIS_Z * 2 + 1] << 4;
		...
		if (data[KXTJ2_1009_AXIS_X] & 0x80) {
			data[KXTJ2_1009_AXIS_X] = ~data[KXTJ2_1009_AXIS_X];
			data[KXTJ2_1009_AXIS_X] &= 0xff;
			data[KXTJ2_1009_AXIS_X] += 1;
			data[KXTJ2_1009_AXIS_X] = -data[KXTJ2_1009_AXIS_X];
		}
		if (data[KXTJ2_1009_AXIS_Y] & 0x80) {
			data[KXTJ2_1009_AXIS_Y] = ~data[KXTJ2_1009_AXIS_Y];
			data[KXTJ2_1009_AXIS_Y] &= 0xff;
			data[KXTJ2_1009_AXIS_Y] += 1;
			data[KXTJ2_1009_AXIS_Y] = -data[KXTJ2_1009_AXIS_Y];
		}
		if (data[KXTJ2_1009_AXIS_Z] & 0x80) {
			data[KXTJ2_1009_AXIS_Z] = ~data[KXTJ2_1009_AXIS_Z];
			data[KXTJ2_1009_AXIS_Z] &= 0xff;
			data[KXTJ2_1009_AXIS_Z] += 1;
			data[KXTJ2_1009_AXIS_Z] = -data[KXTJ2_1009_AXIS_Z];
		}
...
static int KXTJ2_1009_ReadOffset(struct i2c_client *client, s8 ofs[KXTJ2_1009_AXES_NUM])
{
	...
	err = KXTJ2_1009_i2c_read_block(client, KXTJ2_1009_REG_OFSX, ofs, KXTJ2_1009_AXES_NUM);
	if (err)
		GSE_ERR("error: %d\n", err);
#endif
...
static int KXTJ2_1009_CheckDeviceID(struct i2c_client *client)
{
	...
	res = KXTJ2_1009_i2c_read_block(client, KXTJ2_1009_REG_DEVID, databuf, 0x1);
	if (res < 0)
		goto exit_KXTJ2_1009_CheckDeviceID;

	GSE_LOG("KXTJ2_1009_CheckDeviceID %d done!\n ", databuf[0]);
...
static int KXTJ2_1009_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if (KXTJ2_1009_i2c_read_block(client, addr, databuf, 0x01)) {
	...
	res = KXTJ2_1009_i2c_write_block(client, KXTJ2_1009_REG_POWER_CTL, databuf, 0x2);
	if (res < 0) {
...
static int KXTJ2_1009_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	if (KXTJ2_1009_i2c_read_block(client, KXTJ2_1009_REG_DATA_FORMAT, databuf, 0x01)) {
	...
	res = KXTJ2_1009_i2c_write_block(client, KXTJ2_1009_REG_DATA_FORMAT, databuf, 0x2);
	if (res < 0)
		return KXTJ2_1009_ERR_I2C;
...
static int KXTJ2_1009_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	if (KXTJ2_1009_i2c_read_block(client, KXTJ2_1009_REG_BW_RATE, databuf, 0x01)) {
	...
	res = KXTJ2_1009_i2c_write_block(client, KXTJ2_1009_REG_BW_RATE, databuf, 0x2);
	if (res < 0)
		return KXTJ2_1009_ERR_I2C;
	mdelay(1);
...
static ssize_t show_sensordata_value(struct device_driver *ddri, char *buf)
{
	...
	/* KXTJ2_1009150_ReadRawData(client, strbuf); */
	return snprintf(buf, PAGE_SIZE, "%s\n", strbuf);
}
...
static ssize_t show_sensorrawdata_value(struct device_driver *ddri, char *buf, size_t count)
{
	...
	/* KXTJ2_1009150_ReadSensorData(client, strbuf, KXTJ2_1009150_BUFSIZE); */
...
static int gsensor_set_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	/*Fix Me*/
	GSE_LOG("KXTJ2_100922e set delay = (%d) OK!\n", value);
...
static int KXTJ2_1009_i2c_remove(struct i2c_client *client)
{
	...
	if (err != 0)
		GSE_ERR("KXTJ2_1009150_delete_attr fail: %d\n", err);
